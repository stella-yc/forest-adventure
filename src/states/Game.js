...
export default class extends Phaser.State {
  ...
  update () {
    this.setPhysicsCollisions()
    this.orderWorldLayers()
...
/// *********** HELPER FUNCTIONS ************** ///
  ...
  scoreGems (number) {
    ...
    for (let i = 1; i <= number; i++) {
      let scoreGem = this.gems.create(140 + (22 * i), 10, 'gem')
  ...
  setPhysicsCollisions () {
    const collisionPairs = [
      [this.player, this.groundLayer],
      [this.cherries, this.groundLayer],
      [this.bots, this.groundLayer],
      [this.cat, this.groundLayer],
      [this.gems, this.groundLayer],
      [this.trees, this.groundLayer],
      [this.player, this.hiddenPlatforms]
    ]
    collisionPairs.forEach(pair =>
      this.game.physics.arcade.collide(pair[0], pair[1])
    )
  }
